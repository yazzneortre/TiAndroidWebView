...
#define TAG "metavi.android.wv"

using namespace v8;

static Persistent<Object> bindingCache;

static void Natwebview_getBinding(const FunctionCallbackInfo<Value>& args)
{
	Isolate* isolate = args.GetIsolate();
	EscapableHandleScope scope(isolate);

	if (args.Length() == 0) {
		titanium::JSException::Error(isolate, "Natwebview.getBinding requires 1 argument: binding");
	...
	titanium::bindings::BindEntry *extBinding = ::NatwebviewBindings::lookupGeneratedInit(
...
static void Natwebview_init(Local<Object> exports, Local<Context> context)
{
	...
	Local<FunctionTemplate> constructor = FunctionTemplate::New(isolate, Natwebview_getBinding);
	exports->Set(String::NewFromUtf8(isolate, "getBinding"), constructor->GetFunction(context).ToLocalChecked());
}

static void Natwebview_dispose(Isolate* isolate)
{
	HandleScope scope(isolate);
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::NatwebviewBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry NatwebviewBinding = {
	"metavi.android.wv",
	Natwebview_init,
	Natwebview_dispose
};
...
Java_metavi_android_wv_NatwebviewBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("metavi.android.wv", &NatwebviewBinding);
	titanium::KrollBindings::addExternalLookup(&(::NatwebviewBindings::lookupGeneratedInit));
}
